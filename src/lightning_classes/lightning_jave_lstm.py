...
from src.models.jave import JAVEInteraction
from src.lightning_classes import SpanF1Measure

logger = logging.getLogger(__name__)


class JaveLstm(pl.LightningModule):
    def __init__(self, hparams):
        ...
        self.w2i_word, _ = load_vocabulary(os.path.join(self.hparams.vocab_path, "vocab.word"))
        self.embedding = nn.Embedding(len(self.w2i_word), 300)
        self.lstm = nn.LSTM(
            input_size=300, hidden_size=128, num_layers=2, bidirectional=True, batch_first=True, dropout=0.1
        )

        self.jave = JAVEInteraction(
    ...
    def forward(self, inputs):
        ...
        outputs = self.embedding(inputs[1])
        outputs, _ = self.lstm(outputs)

        sequence_length = torch.sum(inputs[3], -1)

        cls_txt_features = torch.zeros((outputs.size(0), 256), device=self.device)

        return self.jave(global_feats,
                         region_feats,
                         cls_txt_features,
                         outputs,
                         inputs[3],
