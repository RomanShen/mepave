...
class JAVEInteraction(nn.Module):
    def __init__(self,
    ...
    def forward(self,
        ...
        global_img_features = torch.zeros_like(global_img_features).to(dtype=next(self.parameters()).dtype)
        regional_img_features = torch.zeros_like(regional_img_features).to(dtype=next(self.parameters()).dtype)
