...
from src.lightning_classes import JaveBert
...
def run(cfg):
    ...
    wandb_logger = WandbLogger(
        name="jave_bert_" + datetime.datetime.now().strftime("%Y-%m-%d_%H:%M:%S"),
        save_dir=output_dir,
        project="mepave"
    )
    ...
    checkpoint_callback = ModelCheckpoint(
        ...
        dirpath=output_dir,
        filename="jave_bert-{epoch:02d}-{validation/f1:.2f}",
        save_top_k=3,
        mode="max",
    )
...
@hydra.main(config_path="conf", config_name="config_jave_bert")
